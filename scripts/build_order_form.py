...
def inp(cell):
    ws[cell].fill = PatternFill('solid', fgColor=EDIT)
...
# سطر المرتجع (كمية سالبة) — يتلوّن برتقالي حتى ما ينقرأ غلط
ws.conditional_formatting.add('A{}:G{}'.format(R0, R1), FormulaRule(
    formula=['AND(ISNUMBER($C{0}),$C{0}<0)'.format(R0)],
    fill=PatternFill('solid', bgColor='FFE0C2'), font=F(11, True, '9A4A00')))
...
STEPS = [
    ...
    ('sec', 'المرتجع'),
    ('6', 'البضاعة الراجعة تنكتب سطر عادي بنفس الورقة، بتاريخ اليوم اللي رجعت فيه، '
          'و**الكمية بالسالب**: «‎-5». السطر بيتلوّن برتقالي لحاله عشان ما ينقرأ غلط.'),
    ('7', 'الإفرادي بسطر المرتجع = السعر اللي انباعت فيه، مش سعر الكتالوج. '
          'وإذا الراجع مكسور أو مفتوح وبدك تخصم عليه، اكتب سعر أقل — هاد اللي رح يترجع للمقاول.'),
    ('8', 'المستلم بسطر المرتجع = اللي رجّع البضاعة.'),
    ('sec', 'المدقِّق — عمر المصري أو أبو علي'),
    ('9', 'التدقيق من الورقة الأصلية، مش من الشاشة. امسك الورقة واقرأ منها.'),
    ('10', 'كل بند تتأكد منه (البند صح والعدد صح) — اختر اسمك بخانة «المدقق» لهذا البند.'),
    ('11', 'العدّاد فوق ما بيصير أخضر إلا لما كل بند مُدخل يكون عليه اسم مدقِّق.'),
    ('12', 'ابعث الملف + صورة الورقة الأصلية للأستاذ.'),
    ('13', 'وقت الطباعة: حدّد المدى المعبّى فقط، لأنه الورقة فيها 200 سطر فاضي جاهزين للورشة كلها.'),
    ('sec', 'الفكرة باختصار'),
